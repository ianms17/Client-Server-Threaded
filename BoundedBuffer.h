...
private:
	...
	mutex m;
	condition_variable data_available;	// wait for pop, signaled by push
	condition_variable slot_available;	// wait for push, signaled by pop
...
public:
	...
	void push(char* data, int len) {
		...
		//3. Then push the vector at the end of the queue, race condition check
		q.push(d);
		l.unlock();
	...
	int pop(char* buf, int bufcap) {
		...
		unique_lock<mutex> l(m);
		data_available.wait(l, [this]{return q.size() > 0;});	// lambda checks that queue isnt empty
		...
		//4. Wake up any potentially sleeping push() threads
